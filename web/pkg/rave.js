...
export class WasmMachine {
    static __wrap(ptr) {
        const obj = Object.create(WasmMachine.prototype);
        obj.__wbg_ptr = ptr;
        WasmMachineFinalization.register(obj, obj.__wbg_ptr, obj);
        return obj;
    }
    __destroy_into_raw() {
     ...
     * @returns {WasmMachine}
     */
    static boot(firmware, kernel, device_tree, memory_size) {
        ...
        const ptr2 = passArray8ToWasm0(device_tree, wasm.__wbindgen_malloc);
        const len2 = WASM_VECTOR_LEN;
        const ret = wasm.wasmmachine_boot(ptr0, len0, ptr1, len1, ptr2, len2, memory_size);
        if (ret[2]) {
    ...
    push_uart_input(input) {
        ...
        const len0 = WASM_VECTOR_LEN;
        wasm.wasmmachine_push_uart_input(this.__wbg_ptr, ptr0, len0);
    }
    /**
     ...
     * @returns {WasmMachine}
     */
    static raw(image, memory_size) {
        const ptr0 = passArray8ToWasm0(image, wasm.__wbindgen_malloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.wasmmachine_raw(ptr0, len0, memory_size);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
        return WasmMachine.__wrap(ret[0]);
    ...
    register_snapshot() {
        ...
        let deferred1_1;
        try {
            const ret = wasm.wasmmachine_register_snapshot(this.__wbg_ptr);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
     ...
     * @returns {string}
     */
    run_chunk(instructions) {
        let deferred2_0;
        let deferred2_1;
        try {
            const ret = wasm.wasmmachine_run_chunk(this.__wbg_ptr, instructions);
            var ptr1 = ret[0];
            var len1 = ret[1];
            if (ret[3]) {
                ptr1 = 0; len1 = 0;
     ...
     */
    take_uart_output() {
        const ret = wasm.wasmmachine_take_uart_output(this.__wbg_ptr);
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('rave_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
